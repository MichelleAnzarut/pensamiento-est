...
Otro escenario usual de pruebas de hipótesis ocurre cuando queremos comparar
dos o más grupos de datos, aunque no neceariamente se trate de grupos formados
por aplicación de un tratamiento al azar. Por ejemplo, quizá quiséramos probar
con una muestra si los capturistas del centro A son más rápidos o más lentos
que los del centro B. 

En este caso, la hipótesis nula de la prueba de permutaciones es la siguiente:

- Todos los grupos tienen el mismo proceso generador de datos.

La explicación de usar pruebas de permutaciones es entonces como sigue:

- Tenemos los datos del grupo A y el grupo B.
- Bajo la hipótesis nula, si permutamos al azar las etqiuetas A y B en los datos,
obtenemos datos que son igualmente verosímiles que la muestra original que obtuvimos.
...
- Bajo nuestra hipótesis nula (proporciones iguales), la etiqueta del grupo no tiene
información acerca de la probabilidad de observar que toma azúcar o no. De forma
que podríamos 

producimos una cantidad grande (por ejemplo 10 mil o más) de muestras permutando las etiquetas de los grupos.
