...
:::callout-note
En estimación, cuando queremos estimar un valor poblacional $m$,
mediante un estimador basado en una muestra, buscamos que su distribución
de muestreo esté altamente concentrada alrededor del valor poblacional $m$, de
manera que sea altamente probable obtener una estimación cercana a $m$
:::
...
compararemos el cociente de las medianas menos uno (variación porcentual):

Podemos adaptar el bootstrap en este caso para dos grupos: hacemos remuestreo de cada grupo, comparamos
diferencias, y repetimos. En primer lugar, nuestro estimador puntual es:
...
```{r, message = FALSE}
muestra_boot(fusion, nv.vv, time, median, VV / NV - 1) |> round(2)
```

Que implica una reducción de 30% en la mediana del tiempo de fusión para
el grupo con información verbal. Ahora construimos un intervalo de confianza:

La distribución de remuestreo es:

```{r, fig.width = 4, fig.height = 3}
reps_boot <- map_dbl(1:2000, ~ muestra_boot(fusion, nv.vv, time, mean, VV / NV - 1))
...
Y esta sería una forma de presentar nuestros resultados: 

- Hay probabilidad considerable de que el efecto
de este tratamiento sea marginal (una reducción de 10\%), aunque lo más probables es que tenga un efecto
consdierable (reducción alrededor de 50\% del tiempo de fusión).

Nótese que no hay una fórmula simple (?) para construir este intervalo para
una estadística como el cociente de medianas.
...
comparando la media de nuestras replicaciones bootstrap con el valor muestral que obtuvimos (ver @timboot14).
Si el tamaño del sesgo es grande comparado con la dispersión de la distribución bootstrap (por ejemplo, más de 20\% de la desviación estándar, @bootefron), generalmente consideramos
que bajo el diseño actual el estimador que estamos usando no es apropiado.
